...
!!$omp end critical
  ...
  subroutine getInitValue_logical(filename,variable,value,err,default)
    implicit none
    character(len=*), intent(in) :: filename, variable
    ...
    if (error.eq.0) then
       ...
       if (ConversionError.ne.0) then
          write(errormsg,*) 'getInitValue_real: error at line ',lineNumber, &
             ' in file "',trim(filename),'". Logical expected but "', &
             trim(string),'" found.'
          error = -2
       end if
    end if
  ...
  end subroutine getInitValue_logical
